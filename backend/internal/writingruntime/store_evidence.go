...
func (store *StoreRolloutEvidence) Record(ctx context.Context, evidence RuntimeEvidence) error {
	...
	return store.recorder.RecordRuntimeEvidence(ctx, writingstore.RuntimeEvidenceRecord{
		EvidenceID: evidence.EvidenceID, RunID: evidence.Identity.RunID,
		NodeID: evidence.Identity.NodeID, Attempt: evidence.Identity.Attempt,
		Kind: evidence.Kind, Payload: payload, OccurredAt: evidence.RecordedAt,
	})
}
