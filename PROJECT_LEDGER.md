...
| E-051 | 代码+验收 | **AR-012 异源 Claim 复核分块策略 + 模型用途路由 + Passkey 注册即同步**：claimverify 分块验证（10 句/块，chunk→global 索引映射，失败块降级跳过，5 次重试 30s 退避）；真实 GLM-5.3 推理模型 e2e（20 句分 2 块，100 秒完成，10 supported + 10 partial）；迁移 117（model_configs.purpose = generation/verification/embedding）+ Admin 模型配置 UI 按用途绑定；Passkey BE/BS 标志解析→真实 device_type（syncable/single_device）+ backed_up 落库（参数绑定，重注册更新），注册/列表全链路 slog，前端同步徽章；移动端 admin 侧边栏 + 个人中心 SectionContainer 适配；迁移表名修复（writing_traces→agent_traces、evaluation_tasks→evaluation_samples+evaluation_sets，双仓 118/114）；双仓 README + docs/04 更新 | `backend/internal/arreview/claimverify/`、迁移 117、`backend/internal/server/webauthn.go`、`frontend/src/pages/personal/account-section.tsx`、`model-configs.tsx`、`admin-layout.tsx`、迁移 114 |
