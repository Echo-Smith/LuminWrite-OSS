...
export function EvalRuns({ runs, suites, candidates, onCreateRun, canMutate = false }: {
  ...
  return (
    <div className="space-y-7">
      ...
      <section className="grid gap-4 border-y bg-muted/30 py-5 dark:border-border dark:bg-muted/20 lg:grid-cols-[1fr_1fr_0.65fr_auto] lg:items-end lg:px-5">
        ...
        <Button className="min-h-11 gap-2 bg-foreground text-[color:var(--desk-canvas)] hover:bg-foreground/85 dark:bg-foreground dark:text-background" disabled={creating || !suiteId || !candidateId || !canMutate} onClick={createRun}>{creating ? <Loader2 className="h-4 w-4 animate-spin" /> : <Play className="h-4 w-4" />}{canMutate ? "启动 Shadow Run" : "只读权限"}</Button>
      </section>
      ...
      {runs.length === 0 ? <EvalEmptyState icon={GitCompareArrows} title="尚无运行记录" description="冻结 suite 和 candidate 后启动首次 shadow run。任务运行在后台异步执行。" /> : (
        <div className="space-y-0 border-y dark:border-border">{runs.map((run) => {
