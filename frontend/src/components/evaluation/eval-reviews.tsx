    ...
    <div className="space-y-7">
      ...
      {items.length === 0 ? <EvalEmptyState icon={ClipboardCheck} title="还没有评审记录" description="运行成功后会产生机评；人工评审可以通过上方中文 Excel 表导入。" /> : (
        <div className="space-y-8">{Object.entries(groups).map(([outputId, reviews]) => {
          ...
          return <section key={outputId} className="border-y border-border/60 dark:border-border">
            <header className="flex flex-col gap-3 bg-muted/30 px-4 py-3 dark:bg-muted/20 sm:flex-row sm:items-center sm:justify-between"><div><p className="font-mono text-xs">{outputId}</p><p className="mt-1 text-xs text-muted-foreground">case {reviews[0]?.caseId} · run {reviews[0]?.runId}</p></div><div className="flex flex-wrap items-center gap-2"><EvalStatus value={reviews[0]?.outputStatus ?? "unknown"} />{!reviews[0]?.contentAvailable && <span className="inline-flex min-h-7 items-center gap-1.5 rounded-full border border-border bg-background px-2.5 text-xs text-muted-foreground"><EyeOff className="h-3.5 w-3.5" />私有正文未下发</span>}{disagreement && <span className="inline-flex min-h-7 items-center gap-1.5 rounded-full bg-amber-700/10 px-2.5 text-xs font-semibold text-amber-800 dark:text-amber-300"><Gavel className="h-3.5 w-3.5" />{arbitration === "resolved" ? "分歧已仲裁" : "存在分歧·待仲裁"}</span>}</div></header>
