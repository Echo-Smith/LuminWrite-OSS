/**
 * WP4 三大写作流程选择测试（docs/28-wp4-pilot-scenarios.md）：
 * - 三流程各自 plan 构建的节点形状/顺序（docs/28 节点图 → 实际 artifact 词汇）
 ...
 * - UI 接线：flow-picker 挂在写作入口，payload 携带 flow
 */
...
test("三个流程各自产出 docs/28 节点图对应的 plan 节点形状与顺序", () => {
...
test("缺省 flow = long_form，默认行为与现状等价", () => {
  ...
  assert.deepEqual(WRITING_FLOW_TYPES, ["long_form", "multi_material", "faithful_rewrite"]);
...
test("写作入口提供流程选择并把所选流程注入 startWriting 数据流", () => {
  ...
  assert.match(composer, /<FlowPicker value=\{flow\} onChange=\{setFlow\} compact=\{compact\} \/>/);
